...
resource "tls_self_signed_cert" "ca" {
  ...
  subject {
    common_name  = "Global WAF Root CA"
    organization = "Global WAF ${var.domain_name} Org"
  }
...
resource "tls_cert_request" "server" {
  ...
  dns_names       = ["www.${var.domain_name}"] # SAN
  subject {
    common_name  = "www.${var.domain_name}"
    organization = "Global ${var.domain_name} Org"
  }
}
...
resource "local_file" "ca_cert_pem" {
  filename        = "${path.module}/.key/waf_ca_crt.pem"
  content         = tls_self_signed_cert.ca.cert_pem
  file_permission = "0644"
}

resource "local_file" "server_cert_pem" {
  filename        = "${path.module}/.key/waf_server_crt.pem"
  content         = tls_locally_signed_cert.server.cert_pem
  file_permission = "0644"
}

resource "local_file" "server_key_pem" {
  filename        = "${path.module}/.key/waf_server_key.pem"
